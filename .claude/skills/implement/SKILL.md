...
| `UPDATE_FEATURE_REGISTRY` | init-code-wiki (refresh mode) | Terminal — done |
...
1. Follow the `init-code-wiki` skill instructions **in refresh mode** — say so explicitly when
   ...
   This refresh is the last code-wiki write `/implement` makes. Ticket-by-ticket reality upkeep
   happens later, post-merge, in closeout's `closeout-feat-recorder` sortie — not here.
