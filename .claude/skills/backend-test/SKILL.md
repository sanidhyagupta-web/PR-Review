...
6. Run tests using `pytest` (see Running below)

## Running

This project has no build tool or test runner config beyond `conftest.py` (which adds the repo
root to `sys.path` so `ingestion.*`-style imports resolve without an installed package). There is
no lint/format/type-check tooling configured (no `ruff`/`flake8`/`black`/`mypy` config) and no CI
job runs tests — the only workflow in `.github/workflows/` is the Claude PR review bot.
