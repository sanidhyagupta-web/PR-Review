...
2. **Read the feature registry** (if it exists):
   - Read `docs/features/index.md` — match ticket keywords to the Workflow Routing Rules table to identify which feature files are relevant
   ...
   - If `docs/features/index.md` does not exist yet, note it in the plan and run `/init-feature-registry` first
...
Populated from `docs/features/` registry. If registry does not exist, state that here.
