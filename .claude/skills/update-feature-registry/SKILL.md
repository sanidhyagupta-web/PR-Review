...
description: SUPERSEDED — docs/features/ is no longer the store; the code wiki is. The code-wiki equivalent is the closeout stage's closeout-feat-recorder sortie, run per-ticket from apps/server/src/activities/closeout.ts, not a skill invoked by hand. Do not run. Update docs/features/ after a ticket is completed. Updates or creates the relevant per-feature index.md files and keeps the master index in sync so future planning can identify what might break.
